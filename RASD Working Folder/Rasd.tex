...
\begin{itemize}
    \item \textbf{User:} a customer of the service who, after having installed the
    ...
    entry)
    \item \textbf{Authority:} a customer who uses the system based on its role;
    \begin{itemize}
        \item \textbf{Policeman:} after installing the application on his mobile device,
        ...
        accused.
        \item \textbf{Municipal Employee:} through the website, he has access to the
        unsafe areas of his own Municipality to decide road interventions to be
        applied within it
        \item \textbf{Municipal Director:} through the website, he has access to both
...
\begin{table}[H]
    ...
    the unsafe areas proposed by SafeStreets and the third party\\
    \end{minipage} \\
    \hline
...
\begin{table}[H]
    ...
    \begin{enumerate}
        ...
        \item The third party reports the violation valid or not valid\\
    \end{enumerate}
    \end{minipage} \\ \hline
...
In this section an analysis of some critical aspects of the system is provided expoliting Alloy. The focus is on some static constraints, in particular:
\begin{itemize}
    \item It cannot happen that two different third parties have the same Id or two different users have the same fiscal code
    \item In case of a police officer erogates a ticket referred to a violation, the violation must have been verified
    \item Every third parties of the same Municipality can see all and only the signalations on the territory of competence
    \item there may be multiple signalations referring to the same and unique violation. 
\end{itemize}
The prove of validity of evidence and all the checks are excluded from the Alloy analysis, they are taken for granted and after them the parameter 'isValid' of a violation is put on 'True'.
When we says Police Officer mean not only the policemen but anyone in charge and able to issue fines and deal with road violations. Same analogy for Director and Employee 
...
different from another. Both Users and Third Parties can share the password and
